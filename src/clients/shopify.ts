...
export class ShopifyClient implements IShopifyClient {
  ...
  async getAllProducts(): Promise<ShopifyProduct[]> {
    if (!this.authenticated || !this.client) {
      throw new Error('Shopify client not authenticated. Call authenticate() first.');
    }
    ...
    try {
      ...
      while (hasMore) {
        ...
        const response = await this.client.get<{ products: ShopifyApiProduct[] }>(
          '/products.json',
          { params }
        );
  ...
  async updateProductPrice(
    ...
    if (!variantId || typeof variantId !== 'number' || variantId <= 0 || !Number.isInteger(variantId)) {
    ...
    try {
      ...
      throw new Error(`Failed to update Shopify product price: ${errorMessage}`);
    }
  }
  ...
  private transformProduct(product: ShopifyApiProduct): ShopifyProduct {
    return {
      id: product.id,
      variants: product.variants.map((variant) => this.transformVariant(variant)),
    };
  }
