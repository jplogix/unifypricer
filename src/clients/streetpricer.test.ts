...
describe("StreetPricerClient", () => {
	...
	beforeEach(() => {
		...
		(axios.create as ReturnType<typeof mock>).mockReturnValue(
			mockAxiosInstance,
		);
	...
	describe("authenticate", () => {
		test("should authenticate successfully with valid credentials", async () => {
			...
			// The client sends form-encoded credentials (URLSearchParams). Accept both object and params in tests.
			expect(mockAxiosInstance.post).toHaveBeenCalledWith(
				"/auth/token",
				expect.anything(),
			);
			const calledArg = (mockAxiosInstance.post as any).mock.calls[0][1];
			if (calledArg && typeof calledArg.get === "function") {
				// URLSearchParams
				expect(calledArg.get("username")).toBe("test-username");
				expect(calledArg.get("password")).toBe("test-password");
			} else {
				// Fallback: object
		...
		test("should throw error when credentials are missing", async () => {
			const clientWithoutCreds = new StreetPricerClient(
				"",
				"",
				"https://api.test.com",
			);
		...
		test("should throw error when authentication fails", async () => {
			...
			(
				axios.isAxiosError as unknown as ReturnType<typeof mock>
	...
	describe("fetchAllProducts", () => {
		...
		test("should fetch products across stores with paginated items", async () => {
			...
			const stores = [{ EbayUserID: "ebay1" }, { EbayUserID: "ebay2" }];
			...
			expect(products).toHaveLength(2);
			expect(products.find((p) => p.id === "s1p1")).toBeTruthy();
			expect(products.find((p) => p.id === "s2p1")).toBeTruthy();
		});
	});
					...
					async (originalApiProducts) => {
						...
						roundTripProducts.forEach((roundTripProduct, index) => {
							...
							const timeDiff = Math.abs(
								roundTripProduct.lastUpdated.getTime() -
									originalProduct.lastUpdated.getTime(),
							);
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (username, password) => {
						...
						const calledArg = (mockAxiosInstance.post as any).mock.calls[0][1];
						if (calledArg && typeof calledArg.get === "function") {
							expect(calledArg.get("username")).toBe(username);
							expect(calledArg.get("password")).toBe(password);
						} else {
							expect(calledArg).toEqual({
								username: username,
								password: password,
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (status, errorMessage) => {
						...
						const escapedError = errorMessage.replace(
							/[.*+?^${}()|[\]\\]/g,
							"\\$&",
						);
						await expect(localClient.fetchAllProducts()).rejects.toThrow(
							new RegExp(
								`StreetPricer authentication failed:.*${escapedError}|${status}`,
							),
						);
