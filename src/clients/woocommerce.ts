...
export class WooCommerceClient implements IWooCommerceClient {
	...
	private readonly minRequestInterval: number = 100; // 100ms = 10 requests/second (conservative for WooCommerce)

	/**
	 * Rate limiting: Wait to ensure we don't overload the WooCommerce API
	 */
	...
	async getAllProducts(): Promise<WooCommerceProduct[]> {
		...
		try {
			...
			while (hasMore) {
				await this.waitForRateLimit();
				const response = await this.client.get<WooCommerceApiProduct[]>(
					"/products",
					{
						params: {
							per_page: perPage,
	...
	async updateProductPrice(productId: number, price: number): Promise<void> {
		...
		try {
			...
			await this.waitForRateLimit();
			await this.client.get<WooCommerceApiProduct>(`/products/${productId}`);
			...
			console.log(
				`[WooCommerce] Updated product ${productId} price to ${price}`,
			);
		} catch (error) {
			const errorMessage = this.getErrorMessage(error);
			console.error(
			...
			throw new Error(
				`Failed to update WooCommerce product price: ${errorMessage}`,
			);
		}
	}
	...
	private transformProduct(product: WooCommerceApiProduct): WooCommerceProduct {
		return {
			...
			regularPrice:
				product.regular_price === undefined ? "0" : product.regular_price,
		};
	}
