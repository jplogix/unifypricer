...
describe('ShopifyClient', () => {
  ...
  describe('authenticate', () => {
    ...
    it('should throw error when authentication test fails', async () => {
      ...
      await expect(client.authenticate('test-shop', 'invalid-token')).rejects.toThrow(
        'Shopify authentication failed: Unauthorized'
      );
    });
  });
  ...
  describe('getAllProducts', () => {
    ...
    it('should handle pagination correctly', async () => {
      ...
      expect(mockAxiosInstance.get).toHaveBeenNthCalledWith(1, '/products.json', {
        params: { limit: 250 },
      });
      expect(mockAxiosInstance.get).toHaveBeenNthCalledWith(2, '/products.json', {
  ...
  describe('updateProductPrice', () => {
    ...
    it('should throw error when not authenticated', async () => {
      ...
      ).rejects.toThrow('Shopify client not authenticated. Call authenticate() first.');
    });
  ...
  describe('error handling', () => {
    ...
    it('should handle string error responses', async () => {
      ...
      await expect(client.authenticate('test-shop', 'invalid-token')).rejects.toThrow(
        'Shopify authentication failed: Invalid request'
      );
    });
    ...
    it('should handle array error responses', async () => {
      ...
      await expect(client.authenticate('test-shop', 'invalid-token')).rejects.toThrow(
        'Shopify authentication failed: Error 1, Error 2'
      );
    });
  });
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (errorMessage) => {
            ...
            await expect(testClient.authenticate('test-shop', 'token'))
              .rejects.toThrow();
          }
        ),
        { numRuns: 50 }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (productId, variantId, newPrice, initialSku, initialTitle) => {
            ...
            const putCallCall = mockAxiosInstance.put.mock.calls.find((call: any[]) => call[0] === `/variants/${variantId}.json`);
