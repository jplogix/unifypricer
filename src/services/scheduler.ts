        ...
        try {
            ...
            for (const config of stores) {
                ...
                if (this.runningSyncs.has(config.storeId)) {
                    this.logger.debug(`Skipping sync for store ${config.storeId}: Sync already in progress.`);
                    continue;
                }

                try {
                    const lastSync = await this.statusRepository.getLatestSyncStatus(config.storeId);
                    const lastSyncTime = lastSync ? new Date(lastSync.timestamp) : new Date(0);

                    // syncInterval is in minutes. Calculate next run time.
                    const nextSyncTime = new Date(lastSyncTime.getTime() + config.syncInterval * 60 * 1000);

                    if (new Date() >= nextSyncTime) {
                        this.logger.info(`Triggering sync for store ${config.storeId} (${config.storeName})`);
                        ...
                        this.syncService.syncStore(store, credentials)
                            ...
                            .catch(err => {
                                this.logger.error(`Sync failed for store ${config.storeId}:`, { error: err });
                            })
                            .finally(() => {
                    ...
                    this.logger.error(`Failed to check status for store ${config.storeId}:`, { error });
                }
            }
        } catch (error) {
