...
import { StoreSyncResult } from "../types/index.js";
import {
	DatabaseConnection,
...
describe("StatusRepository", () => {
	...
	describe("updateProductStatus", () => {
		it("should insert new product status record", async () => {
			...
			const stmt = db.prepare(
				"SELECT * FROM product_status WHERE store_id = ? AND platform_product_id = ?",
			);
	...
	describe("getProductStatus", () => {
		...
		it("should filter by status when provided", async () => {
			const repricedRecords = await statusRepo.getProductStatus(
				"test-store",
				"repriced",
			);
			const pendingRecords = await statusRepo.getProductStatus(
				"test-store",
				"pending",
			);
	...
	describe("clearOldProductStatus", () => {
      ...
      `).run(
			...
			const deletedCount = await statusRepo.clearOldProductStatus(
				"test-store",
				30,
			);
					...
					async (updates) => {
						...
						for (const [id, update] of storedUpdates.entries()) {
							...
							if (!record) {
								...
								console.error(
									"Available records:",
									allRecords.map((r) => r.platformProductId),
								);
								throw new Error(`Record not found for id: ${id}`);
							}

							try {
								expect(record!.streetpricerProductId).toBe(
									update.streetPricerId,
								);
								expect(record!.sku).toBe(update.sku);
								expect(record!.status).toBe(update.status);
								expect(record!.currentPrice).toBeCloseTo(
									update.currentPrice,
									3,
								);
