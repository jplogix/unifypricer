...
import { ConfigRepository, createStoreConfig, getDecryptedCredentials } from './config';
...
describe('ConfigRepository', () => {
  ...
  describe('saveStoreConfig and getStoreConfig', () => {
    ...
    test('should update existing store configuration', () => {
      ...
      const config1 = createStoreConfig('store-1', 'Store 1', 'woocommerce', credentials1, 60, true, testEncryptionKey);
      
      repo.saveStoreConfig(config1);
      
      // Update with new credentials
      const credentials2 = { apiKey: 'key-2' };
      const config2 = createStoreConfig('store-1', 'Store 1 Updated', 'shopify', credentials2, 120, false, testEncryptionKey);
    ...
    test('should handle complex credential objects', () => {
      ...
      const config = createStoreConfig('store-1', 'Complex Store', 'shopify', credentials, 60, true, testEncryptionKey);
      repo.saveStoreConfig(config);
  ...
  describe('getAllStoreConfigs', () => {
    ...
    test('should return all store configurations', () => {
      const config1 = createStoreConfig('store-1', 'Store 1', 'woocommerce', { key: 'val1' }, 60, true, testEncryptionKey);
      const config2 = createStoreConfig('store-2', 'Store 2', 'shopify', { key: 'val2' }, 60, true, testEncryptionKey);
      const config3 = createStoreConfig('store-3', 'Store 3', 'woocommerce', { key: 'val3' }, 60, true, testEncryptionKey);
      ...
      expect(configs.map(c => c.storeId).sort()).toEqual(['store-1', 'store-2', 'store-3']);
    });

    test('should return stores sorted by name', () => {
      const config1 = createStoreConfig('store-1', 'Zebra Store', 'woocommerce', { key: 'val1' }, 60, true, testEncryptionKey);
      const config2 = createStoreConfig('store-2', 'Alpha Store', 'shopify', { key: 'val2' }, 60, true, testEncryptionKey);
      const config3 = createStoreConfig('store-3', 'Beta Store', 'woocommerce', { key: 'val3' }, 60, true, testEncryptionKey);
    ...
    test('should decrypt credentials for all stores', () => {
      ...
      const decrypted1 = getDecryptedCredentials(configs.find(c => c.storeId === 'store-1')!, testEncryptionKey);
      const decrypted2 = getDecryptedCredentials(configs.find(c => c.storeId === 'store-2')!, testEncryptionKey);
  ...
  describe('deleteStoreConfig', () => {
    test('should delete existing store configuration', () => {
      const config = createStoreConfig('store-1', 'Store 1', 'woocommerce', { key: 'val' }, 60, true, testEncryptionKey);
      repo.saveStoreConfig(config);
  ...
  describe('createStoreConfig helper', () => {
    ...
    test('should create config with custom values', () => {
      const config = createStoreConfig('store-1', 'Store 1', 'shopify', { key: 'val' }, 120, false, testEncryptionKey);
    ...
    test('should encrypt credentials', () => {
      ...
      const config = createStoreConfig('store-1', 'Store 1', 'woocommerce', credentials, 60, true, testEncryptionKey);
  ...
  describe('getDecryptedCredentials helper', () => {
    ...
    test('should handle empty credentials object', () => {
      const config = createStoreConfig('store-1', 'Store 1', 'woocommerce', {}, 60, true, testEncryptionKey);
