...
export const config = {
	...
	streetPricer: {
		apiUrl:
			process.env.STREETPRICER_API_URL || "https://api.streetpricer.com/api/v1",
	...
	database: {
		...
		path:
			...
			path.join(__dirname, "../../data/price-sync.db"), // SQLite fallback
		type: (process.env.DATABASE_URL ? "postgres" : "sqlite") as
			| "postgres"
			| "sqlite",
	},
...
export function validateConfig(): void {
	const required = [{ key: "ENCRYPTION_KEY", value: config.encryption.key }];
