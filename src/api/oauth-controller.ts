...
export class OAuthController {
	...
	initiateShopifyOAuth(req: Request, res: Response): void {
		const { shop, storeName } = req.query;
		...
		if (
			...
			res.status(500).json({
				...
				message:
					"Backend Shopify API credentials not configured. Please add SHOPIFY_API_KEY and SHOPIFY_API_SECRET to your backend .env file and restart the server.",
				help: {
					howToSetup:
						"Create a Shopify app at https://partners.shopify.com/ to get your API credentials",
	...
	initiateWooCommerceOAuth(req: Request, res: Response): void {
		...
		if (!clientId || !clientSecret) {
			...
			res.status(500).json({
				...
				message:
					"Backend OAuth credentials not configured. Please add WOOCOMMERCE_OAUTH_CLIENT_ID and WOOCOMMERCE_OAUTH_CLIENT_SECRET to the backend .env file and restart the server.",
				debug: {
		...
		authUrl.searchParams.append(
			"store_name",
			(storeName as string) || "WooCommerce Store",
		);
	...
	async handleWooCommerceCallback(req: Request, res: Response): Promise<void> {
      ...
      `);
		...
		try {
			...
			if (!clientId || !clientSecret) {
				console.error(
					"WooCommerce OAuth credentials missing from environment during callback",
				);
				res.status(500).json({
					error: "WooCommerce OAuth credentials not configured",
					message:
			...
			const tokenResponse = await fetch(
				`${baseUrl}/wp-json/price-sync-oauth/v1/token`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	pollShopifyOAuthResult(req: Request, res: Response): void {
		...
		const { timestamp, ...storeData } = result;
